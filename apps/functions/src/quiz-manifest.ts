...
type QuizQuestionDraft = Omit<QuizQuestion, 'taskFingerprint'>;

type QuizManifestDraft = Omit<QuizManifest, 'questions' | 'taskFingerprint'> & {
...
const handAuthoredQuizManifests: Readonly<Record<string, QuizManifestDraft>> = {
  'quiz-post-dl-p01': {
...
function createAnswerSignature(answer: QuizAnswerValue): string {
  return (Array.isArray(answer) ? [...answer].sort() : [answer]).join('|');
}

function finalizeQuizManifest(draft: QuizManifestDraft): QuizManifest {
  const contextId = `${draft.courseId}:${draft.moduleId}:${draft.postId ?? 'module'}`;
  const questions = draft.questions.map((question, index) => {
    const activityId =
      draft.quizKind === 'post'
        ? `act-${draft.postId}-quiz-${String(index + 1).padStart(2, '0')}`
        : undefined;

    if (draft.quizKind === 'post' && !draft.postId) {
      throw new Error(`Post quiz ${draft.quizId} must reference its stable post ID.`);
    }
    ...
    return {
      ...
      taskFingerprint: createTaskFingerprint({
        contextId,
        datasetId: null,
        expectedReasoningType: `answer:${createAnswerSignature(question.correctAnswer)}`,
        learningObjectiveId: question.questionId,
        taskType: `${draft.quizKind}:${question.type}`,
      }),
    };
  });
  ...
  return {
    ...
    taskFingerprint: createTaskFingerprint({
      contextId,
      datasetId: null,
      expectedReasoningType: `assessment:${questions.map((question) => question.taskFingerprint).join(',')}`,
      learningObjectiveId: draft.quizId,
      taskType: `${draft.quizKind}-quiz`,
    }),
  };
}

function createReleaseQuizManifests() {
  const manifests = handAuthoredQuizManifests;

  return Object.fromEntries(
    Object.entries(manifests).map(([quizId, manifest]) => {
      const enrichedManifest: QuizManifestDraft = {
        ...manifest,
      ...
      return [quizId, finalizeQuizManifest(enrichedManifest)];
    }),
